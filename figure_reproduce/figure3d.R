...
pic[[i]] <- ggplot(out, aes(x=reorder(term, Odds_Ratio), y=Odds_Ratio)) +
  geom_errorbar(aes(ymin=conf.low, ymax=conf.high), 
                width = 0.2,size  = 1,
                position = "dodge", color= "#1e90ff") +
  geom_hline(yintercept = 1, color = "#802417", size = 1,linetype = "dashed") +
  geom_point(size = 4,color = "#508ea2") + 
  coord_flip() +
  theme_ipsum_rc() +
  xlab("")+
  ylab("Odds ratio") +
  #scale_y_continuous(limits = c(0,2))+
  geom_text(aes(x = 1.5,y = conf.high*0.6,label = paste0(round(Odds_Ratio,2),"[",round(conf.low,2),"-",round(conf.high,2),"]")))+
  geom_text(aes(x = 1.4,y = conf.high*0.6,label = paste0("P: ",round(pp,2))))+
  mytheme
}
